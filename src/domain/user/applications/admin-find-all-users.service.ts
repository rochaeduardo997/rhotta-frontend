...
@injectable()
class AdminFindAllUsersService {
  public constructor(@inject(HttpClient) private readonly api: IHttpClient) {}

  public async execute(params?: any) {
    return await this.api.get<TPaginatedResponse<TUser>>('/api/admins/users', { params });
  }
}

container.bind(AdminFindAllUsersService).toSelf();
const adminFindAllUsersService = container.get(AdminFindAllUsersService);
export default adminFindAllUsersService;
